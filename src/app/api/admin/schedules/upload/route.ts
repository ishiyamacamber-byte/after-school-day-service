...
export async function POST(req: Request) {
  ...
  const nextFilePath = await writeScheduleImage(facilityId, month, bytes, media.ext);
  ...
  try {
    ...
    return NextResponse.json({
      ...
      imageUrl: `/api/schedules/image?facilityId=${encodeURIComponent(facilityId)}&month=${encodeURIComponent(month)}`,
    });
  } catch (e) {
